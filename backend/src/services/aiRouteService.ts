 ...
 * @returns 提案されたルートテーマとスポット一覧
 */
export async function suggestCinematicSpotsBase(
    timeLimitMinutes: number,
    startLat: number,
    startLng: number
): Promise<GeminiRouteProposal> {
    if (!ai) {
...
ユーザーの現在地から出発して、制限時間内に戻ってこられる範囲で、最高に「シネマティック（映画的）」な写真や動画が撮れるツーリングルートを1つ企画してください。
...
`;
    ...
    const responseSchema: Schema = {
        ...
        properties: {
            title: {
                type: Type.STRING,
                description: "このルートの魅力的なタイトル（例: '東京湾岸 ナイト・サイバーパンクルート'）",
            },
            theme: {
                type: Type.STRING,
    ...
    try {
        ...
        const data = JSON.parse(responseText) as GeminiRouteProposal;

        // ログ出力（デバッグ用）
        console.log(`[Gemini] Proposal received: ${data.title} (${data.spots.length} spots)`);
